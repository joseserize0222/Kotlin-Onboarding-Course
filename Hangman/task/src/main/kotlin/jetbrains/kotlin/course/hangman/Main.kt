...
fun isComplete(secret : String, currentGuess : String) = (secret.filter { it != ' ' } == currentGuess.filter { it != ' ' })
...
fun generateNewUserWord(secret : String, guess : Char, currentUserWord : String) : String{
    ...
    for(i in secret.indices){
        ...
        else if(userWord[i] != '_') builder.append(userWord[i])
        else builder.append('_')
    }
    return builder.toList().joinToString(" ")
}

fun getHiddenSecret(wordLength: Int) = run{
    val sbl = StringBuilder()
    repeat(wordLength){
        sbl.append('_')
        sbl.append(' ')
    }
    sbl.deleteAt(sbl.length-1).toString()
}
...
fun playGame(secret : String, maxAttemptsCount: Int){
    ...
    while (attempts <= maxAttemptsCount){
        ...
        if(isWon(isComplete(secret, currentGuess), attempts, maxAttemptsCount)){
