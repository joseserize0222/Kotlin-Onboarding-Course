...
fun isCorrectInput(userInput : String, wordLength: Int, alphabet: String) : Boolean{
    ...
    if(!ans) println("All symbols in your guess should be the $alphabet alphabet characters! Try again!")
    return ans
}
...
fun safeUserInput(wordLength: Int, alphabet: String) : String{
    ...
    while(!isCorrectInput(input, wordLength, alphabet)){
        println("Please input your guess. It should be of length $wordLength, and each symbol should be from the alphabet: $alphabet.")
        input = safeReadLine()
    }
    return input
...
fun main() {
    ...
    playGame(generateSecret(wordLength = 4, alphabet = alphabet), wordLength, maxAttemptsCount, alphabet)
}
